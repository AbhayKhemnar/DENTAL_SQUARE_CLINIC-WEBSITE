      ...
      <div className="section-shell grid items-center gap-9 py-10 sm:py-12 lg:min-h-[calc(100vh-80px)] lg:grid-cols-[1.02fr_0.98fr] lg:gap-10 lg:py-16">
        ...
        >
          ...
          <h1 className="mt-4 max-w-4xl text-3xl font-black leading-[1.08] tracking-normal text-slate-950 sm:text-5xl lg:text-6xl">
            Gentle, precise dental care for healthier smiles.
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600 sm:mt-6 sm:text-lg sm:leading-8">
            Dental Square offers modern dental treatments and medicated hydra facial care with a calm, patient-focused experience led by {clinic.doctor}.
          </p>

          <div className="mt-6 grid max-w-xl gap-3 rounded-md border border-blue-100 bg-blue-50/70 p-4 sm:grid-cols-2">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-wider text-blue-700">Doctor</p>
              <p className="mt-1 font-bold text-slate-950">{clinic.doctor}</p>
            </div>
            <div>
              <p className="text-xs font-extrabold uppercase tracking-wider text-blue-700">Clinic Hours</p>
              <p className="mt-1 font-bold text-slate-950">Mon-Sat, Sunday by appointment</p>
            </div>
          </div>
          ...
          <a className="mt-6 inline-flex items-start gap-3 text-sm font-semibold leading-6 text-slate-600 hover:text-blue-700" href={clinic.mapsHref}>
            <FaMapMarkerAlt className="mt-1 shrink-0 text-blue-700" />
        ...
        >
          <div className="absolute -left-5 top-8 hidden h-24 w-24 rounded-md bg-blue-100 sm:block" />
          ...
          <div className="relative overflow-hidden rounded-md border border-slate-200 bg-slate-100 shadow-2xl shadow-slate-300/50">
            {heroImage && (
              <img
                ...
                src={heroImage}
                alt="Dental Square clinic exterior"
                loading="eager"
              />
            )}
            <div className="absolute bottom-0 left-0 right-0 bg-slate-950/82 p-4 text-white backdrop-blur sm:p-5">
              <p className="text-xs font-bold uppercase tracking-wider text-blue-200 sm:text-sm">Multi speciality dental clinic</p>
              <p className="mt-1 text-xl font-black sm:mt-2 sm:text-2xl">Dental Square</p>
            </div>
          </div>
        </motion.div>
      </div>
